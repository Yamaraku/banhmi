...
const DEF_ACCOUNTS = [
  {id:"a0",username:"admin",password:"admin123",name:"Chủ xe 👑",role:"admin",branchId:"b1"},
  {id:"a1",username:"nhanvien",password:"nv123",name:"Nhân viên 1",role:"staff",branchId:"b1"},
];
...
const C = {
  bg: "#1a0a00", card: "#2d1400", accent: "#e8920a", accent2: "#c45c00",
  gold: "#ffd98a", text: "#fff8f0", muted: "#c8a07a",
  green: "#22c55e", red: "#ef4444", blue: "#3b82f6",
  border: "rgba(232,146,10,0.25)", borderStrong: "rgba(232,146,10,0.5)",
};

const inp = (extra={}) => ({
  background:"rgba(255,255,255,0.07)", border:`1.5px solid ${C.border}`,
...
function LoginScreen({onLogin}) {
  ...
  return (
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,#1a0a00,#3d1a00,#7c3c00)`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Nunito',sans-serif"}}>
      <div style={{width:340,background:"rgba(0,0,0,0.5)",borderRadius:20,padding:36,border:`1px solid ${C.borderStrong}`,boxShadow:"0 20px 60px rgba(0,0,0,0.5)"}}>
        ...
        {err && <div style={{background:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.4)",borderRadius:8,padding:"8px 12px",color:"#fca5a5",fontSize:13,marginBottom:14}}>{err}</div>}
        ...
        <div style={{marginTop:18,padding:"12px",background:"rgba(255,255,255,0.04)",borderRadius:8,fontSize:11,color:C.muted}}>
          <div>👑 Admin: <strong style={{color:C.gold}}>admin / admin123</strong></div>
          <div style={{marginTop:4}}>👤 NV: <strong style={{color:C.gold}}>nhanvien / nv123</strong></div>
        </div>
      </div>
    </div>
  );
...
function MainApp({user,accounts,branches,ingredients,products,
  ...
  getDayData,saveDayData,onLogout,
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Nunito',sans-serif",color:C.text,paddingBottom:80}}>
      {/* Header */}
      <div style={{background:`linear-gradient(90deg,${C.bg},#3d1a00)`,padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,zIndex:100}}>
      ...
      <div style={{maxWidth:600,margin:"0 auto"}}>
        {tab==="daily"    && <DailyTab user={user} branchId={selBranch} ingredients={ingredients} products={products} getDayData={getDayData} saveDayData={saveDayData} isAdmin={isAdmin} />}
        {tab==="report"   && <ReportTab user={user} branchId={selBranch} branches={branches} ingredients={ingredients} products={products} getDayData={getDayData} isAdmin={isAdmin} />}
      ...
      {/* Bottom nav */}
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:"rgba(20,8,0,0.97)",borderTop:`1px solid ${C.border}`,display:"flex"}}>
        {tabs.map(t=>(
...
function DailyTab({user,branchId,ingredients,products,getDayData,saveDayData,isAdmin}) {
  const [date,setDate]=useState(today());
  const [data,setData]=useState(null);
  const [section,setSection]=useState("ing"); // ing | sale | vh
  const [saved,setSaved]=useState(false);
  const [vanhanh]=useState(DEF_VANHANH);

  useEffect(()=>{
    getDayData(branchId,date).then(d=>setData(d||{ingredients:{},products:{},vanhanh:{},ghichu:""}));
  },[branchId,date]);
  ...
  const setSale = (id,field,val) => setData(d=>({...d,products:{...d.products,[id]:{...(d.products[id]||{}),price:products.find(p=>p.id===id)?.price,...(d.products[id]||{}),[field]:val}}}));
  const setVH = (id,val) => setData(d=>({...d,vanhanh:{...d.vanhanh,[id]:val}}));

  const chiNL = ingredients.reduce((s,x)=>{
    const r=data.ingredients[x.id]||{}; return s+(Number(r.qty)||0)*(Number(r.unitPrice)||0);
  },0);
  const chiVH = DEF_VANHANH.reduce((s,v)=>s+(Number(data.vanhanh[v.id])||0),0);
  const doanhThu = products.reduce((s,p)=>{
    const r=data.products[p.id]||{}; return s+(Number(r.qty)||0)*(Number(r.price)||p.price);
  },0);
  const tongSP = products.reduce((s,p)=>s+(Number(data.products[p.id]?.qty)||0),0);
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:5,padding:"0 12px"}}>
        ...
        <DailySecBtn id="sale" section={section} setSection={setSection} label="💰 Bán hàng" />
      ...
      {section==="ing" && (
        <DailyCard>
          <div style={{fontSize:12,color:C.muted,marginBottom:12}}>Nhập <strong style={{color:C.gold}}>số lượng mua</strong> + <strong style={{color:C.gold}}>đơn giá</strong> hôm nay</div>
          {/* Header */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 90px 90px 80px",gap:4,marginBottom:6}}>
            {["Nguyên liệu","Số lượng","Đơn giá (đ)","Thành tiền"].map(h=>(
          ...
          {ingredients.map(x=>{
            ...
            const tt=(Number(r.qty)||0)*(Number(r.unitPrice)||0);
            return (
              <div key={x.id} style={{display:"grid",gridTemplateColumns:"1fr 90px 90px 80px",gap:4,marginBottom:8,alignItems:"center"}}>
                <div>
                  <div style={{fontSize:13,fontWeight:700,color:C.text}}>{x.name}</div>
                  <div style={{fontSize:10,color:C.muted}}>đơn vị: {x.unit}</div>
                </div>
                <input type="text" inputMode="decimal" value={r.qty||""} placeholder="0"
                  onChange={e=>setIng(x.id,"qty",e.target.value)}
                  style={inp({padding:"6px 8px",fontSize:13,textAlign:"center"})} />
                <input type="text" inputMode="numeric" value={r.unitPrice||""} placeholder="0"
                  onChange={e=>setIng(x.id,"unitPrice",e.target.value)}
                  style={inp({padding:"6px 8px",fontSize:13,textAlign:"center"})} />
          ...
          <div style={{borderTop:`1px solid ${C.border}`,marginTop:8,paddingTop:10,display:"flex",justifyContent:"space-between"}}>
            ...
            <span style={{fontWeight:900,color:C.accent,fontSize:15}}>{fmt(chiNL)}</span>
          </div>
      ...
      {section==="sale" && (
        <DailyCard>
          <div style={{fontSize:12,color:C.muted,marginBottom:12}}>Nhập <strong style={{color:C.gold}}>số lượng</strong> từng loại đã bán. Giá có thể chỉnh theo ngày.</div>
          {products.map(p=>{
            const r=data.products[p.id]||{};
            const price = Number(r.price)||p.price;
            const qty = Number(r.qty)||0;
            const tt = qty*price;
            return (
              <div key={p.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px",marginBottom:8}}>
                <div style={{fontWeight:800,color:C.text,fontSize:14,marginBottom:8}}>{p.name}</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  <div>
                    <DailyLabel>Số lượng bán</DailyLabel>
                    <input type="text" inputMode="numeric" value={r.qty||""} placeholder="0"
                      onChange={e=>setSale(p.id,"qty",e.target.value)}
                      style={inp({padding:"8px",textAlign:"center",fontSize:15,fontWeight:800})} />
                  </div>
                  <div>
                    <DailyLabel>Giá bán (đ/cái)</DailyLabel>
                    <input type="text" inputMode="numeric" value={r.price!==undefined?r.price:p.price}
                      onChange={e=>setSale(p.id,"price",e.target.value)}
                      style={inp({padding:"8px",textAlign:"center",fontSize:14})} />
                  </div>
                </div>
                {tt>0 && <div style={{marginTop:6,textAlign:"right",color:C.gold,fontWeight:800,fontSize:13}}>= {fmt(tt)}</div>}
              </div>
            );
          })}
          <div style={{borderTop:`1px solid ${C.border}`,marginTop:8,paddingTop:10,display:"flex",justifyContent:"space-between"}}>
            <span style={{fontWeight:800,color:C.muted,fontSize:13}}>Tổng doanh thu: {tongSP} cái</span>
            <span style={{fontWeight:900,color:"#4ade80",fontSize:15}}>{fmt(doanhThu)}</span>
          </div>
        </DailyCard>
      )}
        ...
        <DailyCard>
          {DEF_VANHANH.map(v=>(
            <div key={v.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
              ...
              <div style={{width:130}}>
                <input type="number" min="0" value={data.vanhanh[v.id]||""} placeholder="0 đ"
      ...
      {/* KẾT QUẢ */}
      <div style={{margin:"10px 12px",background:`linear-gradient(135deg,#200800,${C.card})`,borderRadius:14,padding:16,border:`1px solid ${C.borderStrong}`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
          {[[fmt(doanhThu),"Doanh thu","#4ade80"],[fmt(chiNL+chiVH),"Tổng chi","#f87171"],[tongSP+" cái","Đã bán",C.gold]].map(([v,l,c])=>(
            <div key={l} style={{background:"rgba(0,0,0,0.3)",borderRadius:10,padding:"10px 6px",textAlign:"center"}}>
...
// REPORT TAB
// ═══════════════════════════════════════════════════════
function ReportTab({user,branchId,branches,ingredients,products,getDayData,isAdmin}) {
  ...
  const calcDay = data => {
    ...
    const chiNL=ingredients.reduce((s,x)=>{const r=data.ingredients?.[x.id]||{};return s+(Number(r.qty)||0)*(Number(r.unitPrice)||0);},0);
    const chiVH=DEF_VANHANH.reduce((s,v)=>s+(Number(data.vanhanh?.[v.id])||0),0);
    const doanhThu=products.reduce((s,p)=>{const r=data.products?.[p.id]||{};return s+(Number(r.qty)||0)*(Number(r.price)||p.price);},0);
    const tongSP=products.reduce((s,p)=>s+(Number(data.products?.[p.id]?.qty)||0),0);
  ...
  const Card=({children,style={}})=><div style={{background:C.card,borderRadius:14,padding:14,margin:"10px 12px",border:`1px solid ${C.border}`,...style}}>{children}</div>;
  ...
  return (
    <div>
      ...
      {/* Tổng tháng */}
      <div style={{margin:"0 12px",background:`linear-gradient(135deg,#1a0800,#3d1a00)`,borderRadius:14,padding:16,border:`1px solid ${C.borderStrong}`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
          {[[fmt(totals.doanhThu),"💰 Doanh thu","#4ade80"],[fmt(totals.chiNL),"🛒 Chi NL","#f87171"],[fmt(totals.chiVH),"⚙️ Chi VH","#fb923c"],[totals.tongSP+" cái","🥖 Tổng bán",C.gold]].map(([v,l,c])=>(
            <div key={l} style={{background:"rgba(0,0,0,0.3)",borderRadius:10,padding:"10px 12px"}}>
          ...
          : <Card style={{padding:0,overflow:"hidden"}}>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                  <thead>
                    <tr>{["Ngày","Doanh thu","Chi NL","Chi VH","Lời/Lỗ","Bánh"].map(h=>(
                  ...
                  <tbody>
                    {dayEntries.map(([date,d])=>{
                      ...
                      return <tr key={date} style={{background:c.loiLo>0?"rgba(34,197,94,0.05)":c.loiLo<0?"rgba(239,68,68,0.05)":"transparent"}}>
                        ...
                        <td style={{padding:"7px 6px",borderBottom:`1px solid ${C.border}`,textAlign:"center",color:"#f87171"}}>{fmt(c.chiNL)}</td>
      ...
      results.forEach(({d,data})=>{
        ...
        ingredients.filter(x=>x.unit!=="đ"&&x.unit!=="nghìn đ").forEach(x=>{
          ...
          if(nhap>0||dung>0) {
            ...
            stock[x.id]=Math.max(0,truoc+nhap-dung);
            hist[x.id]=[...hist[x.id],{date:d,nhap,dung,conLai:stock[x.id]}];
          }
        });
      });
  ...
  return (
    <div>
      ...
      <div style={{padding:"4px 12px",fontSize:13,color:C.muted}}>
        💡 Để cập nhật tồn kho: vào <strong style={{color:C.gold}}>📝 Nhập ngày</strong> → mục nguyên liệu có thêm ô "Nhập thêm" và "Đã dùng"
      </div>
    </div>
  );
}
...
function SettingsTab({accounts,branches,ingredients,products,saveAccounts,saveBranches,saveIngredients,saveProducts,exportBackup,importBackup}) {
  ...
  const Card=({children,style={}})=><div style={{background:C.card,borderRadius:14,padding:14,margin:"10px 12px",border:`1px solid ${C.border}`,...style}}>{children}</div>;
  const SecBtn=({id,label})=><button onClick={()=>setSec(id)} style={{flex:1,padding:"8px 4px",border:"none",cursor:"pointer",borderRadius:8,background:sec===id?C.accent:"rgba(255,255,255,0.06)",color:sec===id?"#fff":C.muted,fontWeight:700,fontSize:11,fontFamily:"inherit"}}>{label}</button>;

  return (
    <div>
      <div style={{display:"flex",gap:4,padding:"10px 12px 0"}}>
        <SecBtn id="ing"    label="🛒 NL" />
        <SecBtn id="prod"   label="🥖 SP" />
        <SecBtn id="branch" label="🏪 Chi nhánh" />
        <SecBtn id="acc"    label="👤 Tài khoản" />
        <SecBtn id="backup" label="💾 Sao lưu" />
      </div>
...
function BackupSettings({exportBackup,importBackup}) {
  ...
  const Card=({children})=><div style={{background:C.card,borderRadius:14,padding:14,margin:"10px 12px",border:`1px solid ${C.border}`}}>{children}</div>;
...
function IngredientsSettings({ingredients,saveIngredients}) {
  ...
  const [newName,setNewName]=useState(""); const [newUnit,setNewUnit]=useState("kg");
  const [editing,setEditing]=useState(null);

  const add = () => {
    if(!newName.trim()) return;
    const updated=[...list,{id:uid(),name:newName.trim(),unit:newUnit||"cái"}];
    setList(updated); saveIngredients(updated); setNewName(""); setNewUnit("kg");
  };
    ...
    <Card>
      ...
      <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,marginTop:8}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          ...
          <select value={newUnit} onChange={e=>setNewUnit(e.target.value)} style={{...inp({flex:1,minWidth:80,padding:"8px 8px"})}}>
            {UNITS.map(u=><option key={u} value={u}>{u}</option>)}
          </select>
...
function ProductsSettings({products,saveProducts}) {
  ...
  const add = () => {
    if(!newName.trim()) return;
    const u=[...list,{id:uid(),name:newName.trim(),price:Number(newPrice)||15000}];
    setList(u); saveProducts(u); setNewName(""); setNewPrice("15000");
  };
  const del = id => { const u=list.filter(x=>x.id!==id); setList(u); saveProducts(u); };
  const edit = (id,field,val) => { const u=list.map(x=>x.id===id?{...x,[field]:field==="price"?Number(val):val}:x); setList(u); saveProducts(u); };
    ...
    <Card>
          ...
          <div style={{flex:1}}>
            {editing===x.id ? (
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                ...
                <input type="number" value={x.price} onChange={e=>edit(x.id,"price",e.target.value)} style={{...inp({width:90,padding:"5px 8px",fontSize:13,textAlign:"center"})}} />
                <button onClick={()=>setEditing(null)} style={{...btn("#166534",{padding:"5px 10px",fontSize:12})}}>✓</button>
              </div>
            ) : (
              <div style={{display:"flex",alignItems:"center",gap:8}}>
      ...
      <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,marginTop:8}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          ...
          <input type="number" value={newPrice} onChange={e=>setNewPrice(e.target.value)} placeholder="Giá" style={{...inp({width:90,padding:"8px 8px",textAlign:"center"})}} />
        ...
        <div key={a.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px",marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
            <div>
              ...
              <div style={{fontSize:11,color:C.muted}}>@{a.username} · {a.role==="admin"?"👑 Admin":"👤 Nhân viên"}</div>
            </div>
...
export default function Root() {
  ...
  if(!loaded) return (
    <div style={{minHeight:"100vh",background:"#1a0a00",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:12,fontFamily:"'Nunito',sans-serif"}}>
      <div style={{fontSize:48}}>🥖</div>
      <div style={{color:"#ffd98a",fontSize:18,fontWeight:700}}>Đang tải hệ thống...</div>
    </div>
  );
